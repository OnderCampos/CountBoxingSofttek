...
class BoxLoader:
    """Loads tower side faces from JSON annotation files."""
    ...
    def get_sides(tower_path):
        ...
        with open(f"{tower_path}/json/front.json") as f:
            front_side = BoxLoader.create_faces(json.load(f))
        with open(f"{tower_path}/json/right.json") as f:
            right_side = BoxLoader.create_faces(json.load(f))
        with open(f"{tower_path}/json/back.json") as f:
            back_side = BoxLoader.create_faces(json.load(f))
        with open(f"{tower_path}/json/left.json") as f:
            left_side = BoxLoader.create_faces(json.load(f))
